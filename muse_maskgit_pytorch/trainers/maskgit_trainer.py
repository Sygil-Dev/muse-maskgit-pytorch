...
class MaskGitTrainer(BaseAcceleratedTrainer):
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            for imgs, input_ids, attn_mask in iter(self.dl):
                ...
                if not (steps % self.save_results_every):
                    ...
                    if self.on_tpu:
                        self.accelerator.print(f"[E{epoch + 1}][{steps:05d}]{proc_label}: saved to {saved_image}")
                    else:
                        self.info_bar.set_description_str(f"[E{epoch + 1}]{proc_label}: "
                ...
                if met is not None and not (steps % self.log_metrics_every):
                    if self.on_tpu:
                        self.accelerator.print(f"[E{epoch + 1}][{steps:05d}]{proc_label}: metrics:")
                    else:
            ...
            if self.num_train_steps > 0 and self.steps >= int(self.steps.item()):
                if self.on_tpu:
                    self.accelerator.print(f"[E{epoch + 1}][{int(self.steps.item()):05d}]{proc_label}"
        ...
        if self.use_ema:
            self.accelerator.print(
                f"[{steps:05d}]{proc_label}[FINAL]: saving EMA model to {self.results_dir}"
            )
            ema_state_dict = self.accelerator.unwrap_model(self.ema_model).state_dict()
            file_name = (
                f"{maskgit_save_name}.{steps}.ema.pt"
        ...
        self.accelerator.print(f"[{steps:05d}]{proc_label}: saved to {saved_image}")

        if met is not None and not (steps % self.log_metrics_every):
            self.accelerator.print(f"[{steps:05d}]{proc_label}: metrics:")
